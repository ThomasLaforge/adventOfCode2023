...
while(lineIndex < input.length){
    ...
    for (let j = 0; j < line.length; j++) {
        ...
        if(element !== "." && Number.isNaN(parseInt(element))){
            symbols.push({
                value: element,
                i: lineIndex,
                j
...
while(lineIndex < input.length){
    ...
    while (j < line.length) {
        ...
        if(!Number.isNaN(parseInt(element))){
            ...
            j++
            while(j < line.length && !Number.isNaN(parseInt(line[j])) ){
...
const res = numbers.reduce((sum, num) => {
    const isLinkedToSymbol = symbols.find(s => {
        // Same Line
        if(s.i === num.lineIndex){
            // Before
            if(s.j === num.startCol - 1){
                return true
            }
            // After
            if(s.j === num.endCol + 1){
        ...
        // on top Line
        if(s.i === num.lineIndex - 1){
            if(s.j >= num.startCol - 1 && s.j <= num.endCol + 1){
                return true
            }
        }
    ...
    return sum + (isLinkedToSymbol ? num.value : 0)
}, 0)
