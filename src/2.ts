...
const result = inputDayTwo.reduce((sum, curr) => {
    ...
    colors.split(';').forEach(pack => {
        const colorPicks = pack.split(',')
        const colorPicksTrimmed = colorPicks.map(pick => pick.trim())
        colorPicksTrimmed.forEach(pick => {
            const [amountStr, color] = pick.split(" ")
            const amount = parseInt(amountStr)
            if(color === "red" && amount > redObjectif){
                isPossible = false
            }
            else if(color === "blue" && amount > blueObjectif){
                isPossible = false
            }
            else if(color === "green" && amount > greenObjectif){
                isPossible = false
            }
        })
    })

    console.log("is possible", isPossible, id)
    return sum + (isPossible ? id : 0)
}, 0)
